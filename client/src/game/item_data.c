...
u32 get_item_count(u32 item_type_index) {
    ...
    SM_ASSERT(strcmp(token, item_type_str) == 0);
    token = strtok(NULL, ":");
    SM_ASSERT(is_number(token) == true);
    return atoi(token);
}
...
u8 load_item_data() {
    ...
    SM_ASSERT_MSG(result == 0, "first line of item data file must be #item_counts");
...
u8 init_item_data() {
    SM_ASSERT(torso_list              == 0);
    SM_ASSERT(leg_list                == 0);
    SM_ASSERT(side_weapon_list        == 0);
    SM_ASSERT(top_weapon_list         == 0);
    SM_ASSERT(teleport_module_list    == 0);
    SM_ASSERT(charge_module_list      == 0);
    SM_ASSERT(grapple_module_list     == 0);
    SM_ASSERT(drone_list              == 0);
    SM_ASSERT(module_list             == 0);
    ...
    while(strcmp(line_buffer, "#end") != 0) {
        ...
        {   //reading item attribute
            ...
            while(line_buffer[index] == ' ' || line_buffer[index] == '\t')
                index++;
            SM_ASSERT(first_item_read_started == true);
            ...
            else { SM_ASSERT_MSG(false, "attempted to load invalid attribute for a torso item"); }
        }
        else { //next item
            token = strtok(line_buffer, ":");
...
u8 load_leg_data(u32 count) {
    ...
    while(strcmp(line_buffer, "#end") != 0) {
        ...
        {   //reading item attribute
            ...
            else { SM_ASSERT_MSG(false, "attempted to load invalid attribute for a leg item"); }
        }
        else { //next item
            token = strtok(line_buffer, ":");
            strcpy(item_name, token);
...
u8 load_side_weapon_data(u32 count) {
    ...
    while(strcmp(line_buffer, "#end") != 0) {
        ...
        {   //reading item attribute
            ...
            else { SM_ASSERT_MSG(false, "attempted to load invalid attribute for a side weapon item"); }
        }
        else { //next item
            token = strtok(line_buffer, ":");
            strcpy(item_name, token);
...
u8 load_top_weapon_data(u32 count) {
    ...
    while(strcmp(line_buffer, "#end") != 0) {
        ...
        {   //reading item attribute
            ...
            else { SM_ASSERT_MSG(false, "attempted to load invalid attribute for a top weapon item"); }
        }
        else { //next item
            token = strtok(line_buffer, ":");
            strcpy(item_name, token);
...
u8 load_teleport_module_data(u32 count) {
    ...
    while(strcmp(line_buffer, "#end") != 0) {
        ...
        {   //reading item attribute
            ...
            else { SM_ASSERT_MSG(false, "attempted to load invalid attribute for a teleport module item"); }
        }
        else { //next item
            token = strtok(line_buffer, ":");
            strcpy(item_name, token);
...
u8 load_charge_module_data(u32 count) {
    ...
    while(strcmp(line_buffer, "#end") != 0) {
        ...
        {   //reading item attribute
            ...
            else { SM_ASSERT_MSG(false, "attempted to load invalid attribute for a charge module item"); }
        }
        else { //next item
            token = strtok(line_buffer, ":");
            strcpy(item_name, token);
...
u8 load_grapple_module_data(u32 count) {
    ...
    while(strcmp(line_buffer, "#end") != 0) {
        ...
        {   //reading item attribute
            ...
            else { SM_ASSERT_MSG(false, "attempted to load invalid attribute for a grapple module item"); }
        }
        else { //next item
            token = strtok(line_buffer, ":");
            strcpy(item_name, token);
...
u8 load_drone_data(u32 count) {
    ...
    while(strcmp(line_buffer, "#end") != 0) {
        ...
        {   //reading item attribute
            ...
            else { SM_ASSERT_MSG(false, "attempted to load invalid attribute for a drone item"); }
        }
        else { //next item
            token = strtok(line_buffer, ":");
            strcpy(item_name, token);
...
u8 load_module_data(u32 count) {
    ...
    while(strcmp(line_buffer, "#end") != 0) {
        ...
        {   //reading item attribute
            ...
            else { SM_ASSERT_MSG(false, "attempted to load invalid attribute for a module item"); }
        }
        else { //next item
            token = strtok(line_buffer, ":");
            strcpy(item_name, token);
...
item_torso get_torso(i32 i_ID) {
    SM_ASSERT(i_ID < item_count_list[I_TORSO]);
    item_torso temp;
...
item_leg get_leg(i32 i_ID) {
    SM_ASSERT(i_ID < item_count_list[I_LEG]);
    item_leg temp;
...
item_side_weapon get_side_weapon(i32 i_ID) {
    SM_ASSERT(i_ID < item_count_list[I_SIDE_WEAPON]);
...
item_top_weapon get_top_weapon(i32 i_ID) {
    SM_ASSERT(i_ID < item_count_list[I_TOP_WEAPON]);
...
item_teleport_module get_teleport_module(i32 i_ID) {
    SM_ASSERT(i_ID < item_count_list[I_TELEPORT_MODULE]);
...
item_charge_module get_charge_module(i32 i_ID) {
    SM_ASSERT(i_ID < item_count_list[I_CHARGE_MODULE]);
...
item_grapple_module get_grapple_module(i32 i_ID) {
    SM_ASSERT(i_ID < item_count_list[I_GRAPPLE_MODULE]);
...
item_drone get_drone(i32 i_ID) {
    SM_ASSERT(i_ID < item_count_list[I_DRONE]);
    item_drone temp;
...
item_module get_module(i32 i_ID) {
    SM_ASSERT(i_ID < item_count_list[I_MODULE]);
    item_module temp;
